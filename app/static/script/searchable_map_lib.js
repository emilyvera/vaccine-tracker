...
var SearchableMapLib = {
  ...
  initialize: function(options){
    ...
    if (!SearchableMapLib.map) {
      ...
      var hover_template;
      $.get( "static/script/hover.ejs", function( template ) {
  ...
  renderList: function() {
    ...
    if (SearchableMapLib.currentResults.features.length == 0) {
      ...
      var row_content;
      $.get( "static/script/table-row.ejs", function( template ) {
  ...
  createSQL: function() {
    ...
    SearchableMapLib.currentResultsLayer = L.geoJSON(SearchableMapLib.currentResults, {
        pointToLayer: function (feature, latlng) {
          return L.marker(latlng, {icon: SearchableMapLib.getIcon(feature.properties["SafetyRating"])} );
        },
  ...
  // -----custom functions-----
  getIcon: function(safetyRating){
    if (safetyRating == 5) return greenIcon;
    if (safetyRating == 4) return blueIcon;
    if (safetyRating == 3) return orangeIcon;
    if (safetyRating == 2) return yellowIcon;
    return redIcon;
  },
